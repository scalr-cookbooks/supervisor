...
action :start do
  execute "supervisorctl -c '#{node['supervisor']['conffile']}' start #{new_resource.group_name}:*" do
    user "root"
  end
end

action :stop do
  execute "supervisorctl -c '#{node['supervisor']['conffile']}' stop #{new_resource.group_name}:*" do
    user "root"
  end
end

action :restart  do
  execute "supervisorctl -c '#{node['supervisor']['conffile']}' restart #{new_resource.group_name}:*" do
    user "root"
  end
end
